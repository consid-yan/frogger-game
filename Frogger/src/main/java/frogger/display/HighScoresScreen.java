...
public class HighScoresScreen implements Screen {
    ...
    public void paint() {
        ...
        gc.fillText("Frogger Hall of Fame", canvas.getWidth() / 2.0, canvas.getHeight() / 10.0);
        ...
        for (int i = 0; i < scores.length; i++) {
            ...
            gc.fillText(score.getName(), canvas.getWidth() / 3.0, start + i * gap);
            gc.fillText("" + score.getScore(), 2 * canvas.getWidth() / 3.0, start + i * gap);
        }

        // Back to Main Menu
        gc.setTextAlign(TextAlignment.CENTER);
        gc.setFont(new Font("Arial", 28));
        gc.fillText("Back to the Main Menu(Press M)", canvas.getWidth() / 2.0, 4 * canvas.getHeight() / 5.0);
        gc.restore();
    }
}
