...
public class SettingsScreen implements Screen {
    ...
    public void paint() {
        ...
        gc.fillText("Controls", 3 * canvas.getWidth() / 7.0, canvas.getHeight() / 10.0);

        // Controls and Keys
        gc.setFont(new Font("Arial", 20));
        int start = 128, gap = 48; // Set the basic configs
        for (int i = 0; i < CONTROLS.length; i++) {
            gc.fillText(CONTROLS[i], 5 * canvas.getWidth() / 28.0, start + i * gap);
        }
        gc.setFill(Color.GOLD);
        for (int i = 0; i < KEYS.length; i++) {
            gc.fillText(KEYS[i], 5 * canvas.getWidth() / 7.0, start + i * gap);
        }
